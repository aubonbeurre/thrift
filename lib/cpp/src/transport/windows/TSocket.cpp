...
void TSocket::openConnection(struct addrinfo *res) {
  ...
  if (connTimeout_ > 0) {
      throw std::runtime_error("Non-blocking timeouts not supported.");
//     u_long flags(1);
//     if (-1 == ioctlsocket(socket_, FIONBIO, &flags)) {
//       int errno_copy = errno;
...
//     }
  ...
  int ret;
  if (! path_.empty()) {
      throw std::runtime_error("Named pipes not yet supported.");
...
//     ret = connect(socket_, (struct sockaddr *) &address, len);
  ...
  ret = /*WSAPoll(fds, 1, connTimeout_)*/0;
  ...
  if (ret > 0) {
    ...
    socklen_t lon;
    lon = sizeof(val);
    int ret2 = getsockopt(socket_, SOL_SOCKET, SO_ERROR, &val, &lon);
    if (ret2 == -1) {
      int errno_copy = errno;
...
uint32_t TSocket::read(uint8_t* buf, uint32_t len) {
 ...
 try_again:
  ...
  int got = recv(socket_, reinterpret_cast<char *>(buf), len, 0);
...
uint32_t TSocket::write_partial(const uint8_t* buf, uint32_t len) {
  ...
  int b = send(socket_, reinterpret_cast<char const *>(buf + sent), len - sent, flags);
  ++g_socket_syscalls;
...
void TSocket::setLinger(bool on, int linger) {
  ...
  int ret = setsockopt(socket_, SOL_SOCKET, SO_LINGER, reinterpret_cast<char *>(&l), sizeof(l));
  if (ret == -1) {
...
void TSocket::setNoDelay(bool noDelay) {
  ...
  int v = noDelay_ ? 1 : 0;
  int ret = setsockopt(socket_, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<char *>(&v), sizeof(v));
  if (ret == -1) {
    int errno_copy = errno;  // Copy errno because we're allocating memory.
...
void TSocket::setRecvTimeout(int ms) {
  ...
  int ret = setsockopt(socket_, SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<char *>(&r), sizeof(r));
  if (ret == -1) {
    int errno_copy = errno;  // Copy errno because we're allocating memory.
...
void TSocket::setSendTimeout(int ms) {
  ...
  int ret = setsockopt(socket_, SOL_SOCKET, SO_SNDTIMEO, reinterpret_cast<char *>(&s), sizeof(s));
  if (ret == -1) {
    int errno_copy = errno;  // Copy errno because we're allocating memory.
